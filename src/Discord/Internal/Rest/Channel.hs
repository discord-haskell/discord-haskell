...
channelJsonRequest c = case c of
  ...
  (CreateReaction (chan, msgid) emoji) ->
    let e = cleanupEmoji emoji
     in Put
          (channels // chan /: "messages" // msgid /: "reactions" /: e /: "@me")
          R.NoReqBody
          mempty
  ...
  (GroupDMAddRecipient chan (GroupDMAddRecipientOpts uid nick tok)) ->
    Put
      (channels // chan // chan /: "recipients" // uid)
      ( R.ReqBodyJson
          ( object
              [ ("access_token", toJSON tok),
                ("nick", toJSON nick)
              ]
          )
      )
  ...
  (StartThreadFromMessage chan mid sto) ->
    Post
      (channels // chan /: "messages" // mid /: "threads")
      (pure $ R.ReqBodyJson $ toJSON sto)
      mempty
  (StartThreadNoMessage chan sto) ->
    Post
      (channels // chan /: "messages" /: "threads")
      (pure $ R.ReqBodyJson $ toJSON sto)
      mempty
  (JoinThread chan) ->
    Put
      (channels // chan /: "thread-members" /: "@me")
      R.NoReqBody
      mempty
  (AddThreadMember chan uid) ->
    Put
      (channels // chan /: "thread-members" // uid)
      R.NoReqBody
      mempty
  (LeaveThread chan) ->
    Delete
