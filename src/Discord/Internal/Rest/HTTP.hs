...
restLoop ::
  Auth ->
  Chan (String, JsonRequest, MVar (Either RestCallInternalException BL.ByteString)) ->
  Chan T.Text ->
  IO ()
  ...
  where
    loop ratelocker = do
      ...
      case compareRate ratelocker route curtime of
        Locked -> do
          writeChan urls (route, request, thread)
          loop ratelocker
        Available -> do
          let action = compileRequest auth request
...
removeAllExpire ratelocker curtime =
  if M.size ratelocker > 100
    then M.filter (> curtime) ratelocker
    else ratelocker
  ...
  if
      | code == 429 ->
        pure
          ( ResponseTryAgain,
            if global
              then GlobalWait reset
              else PathWait reset
          )
      | code `elem` [500, 502] -> pure (ResponseTryAgain, NoLimit)
      | inRange (200, 299) code ->
        pure
          ( ResponseByteString body,
            if remain > 0 then NoLimit else PathWait reset
          )
      | inRange (400, 499) code ->
        pure
          ( ResponseErrorCode code status (BL.toStrict body),
            if remain > 0 then NoLimit else PathWait reset
          )
