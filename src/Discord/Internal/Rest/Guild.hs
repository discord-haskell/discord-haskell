...
instance ToJSON ModifyGuildIntegrationOpts where
  toJSON ModifyGuildIntegrationOpts {..} = object
    [ (name, val)
    | (name, Just val) <-
      [ ( "expire_grace_period"
        , toJSON <$> pure modifyGuildIntegrationOptsExpireGraceSeconds
        )
      , ( "expire_behavior"
        , toJSON <$> pure modifyGuildIntegrationOptsExpireBehavior
        )
      , ( "enable_emoticons"
        , toJSON <$> pure modifyGuildIntegrationOptsEmoticonsEnabled
        )
      ]
    ]
...
instance ToJSON CreateGuildIntegrationOpts where
  toJSON CreateGuildIntegrationOpts {..} = object
    [ (name, val)
    | (name, Just val) <-
...
instance ToJSON CreateGuildBanOpts where
  toJSON CreateGuildBanOpts {..} = object
    [ (name, val)
    | (name, Just val) <-
      [ ( "delete-message-days"
...
instance ToJSON ModifyGuildRoleOpts where
  toJSON ModifyGuildRoleOpts {..} = object
    [ (name, val)
    | (name, Just val) <-
...
instance ToJSON AddGuildMemberOpts where
  toJSON AddGuildMemberOpts {..} = object
    [ (name, val)
    | (name, Just val) <-
...
instance ToJSON ModifyGuildMemberOpts where
  toJSON ModifyGuildMemberOpts {..} = object
    [ (name, val)
    | (name, Just val) <-
...
createChannelOptsToJSON
  :: T.Text -> [Overwrite] -> CreateGuildChannelOpts -> Value
createChannelOptsToJSON name perms opts = object
  [ (key, val) | (key, Just val) <- optsJSON ]
 where
  optsJSON = case opts of
    CreateGuildChannelOptsText {..} ->
      ...
      , ( "rate_limit_per_user"
        , toJSON <$> createGuildChannelOptsUserMessageRateDelay
        )
...
guildJsonRequest c = case c of
  (CreateGuild opts ) -> Post (guilds) (pure (R.ReqBodyJson opts)) mempty

  (GetGuild    guild) -> Get (guilds // guild) mempty

  (ModifyGuild guild patch) ->
    Patch (guilds // guild) (R.ReqBodyJson patch) mempty

  (DeleteGuild guild) -> Delete (guilds // guild) mempty

  (GetGuildChannels guild) -> Get (guilds // guild /: "channels") mempty

  (CreateGuildChannel guild name perms patch) -> Post
    (guilds // guild /: "channels")
    (pure (R.ReqBodyJson (createChannelOptsToJSON name perms patch)))
    mempty

  (ModifyGuildChannelPositions guild newlocs) ->
    let patch = map
          (\(a, b) -> object [("id", toJSON a), ("position", toJSON b)])
          newlocs
  ...
  (ModifyCurrentUserNick guild name) ->
    ...
    in  Patch (guilds // guild /: "members/@me/nick")
              (R.ReqBodyJson patch)
              mempty

  (AddGuildMemberRole guild user role) ->
    let body = R.ReqBodyJson (object [])
    in  Put (guilds // guild /: "members" // user /: "roles" // role)
            body
            mempty
  ...
  (GetGuildRoles guild) -> Get (guilds // guild /: "roles") mempty
  ...
  (ModifyGuildRolePositions guild patch) ->
    let body =
            map (\(role, pos) -> object ["id" .= role, "position" .= pos]) patch
  ...
  (GetGuildVoiceRegions guild) -> Get (guilds // guild /: "regions") mempty

  (GetGuildInvites      guild) -> Get (guilds // guild /: "invites") mempty
  ...
  (CreateGuildIntegration guild iid opts) ->
    let
      patch =
        object [("type" .= createGuildIntegrationOptsType opts), ("id" .= iid)]
    in  Post (guilds // guild /: "integrations")
             (pure (R.ReqBodyJson patch))
             mempty
  ...
  (GetGuildEmbed guild) -> Get (guilds // guild /: "integrations") mempty
