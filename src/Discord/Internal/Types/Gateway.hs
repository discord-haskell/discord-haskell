...
compileGatewayIntent GatewayIntent {..} =
  sum $
    [ if on then flag else 0
      | (flag, on) <-
          [ (1, gatewayIntentGuilds),
    ...
    case op of
      0 -> do
        ...
        case ejson of
          ...
          _other ->
            Dispatch (InternalUnknownEvent "Dispatch payload wasn't an object" o)
              <$> o .: "s"
...
instance ToJSON GatewaySendableInternal where
  ...
  toJSON (Identify token intent shard) =
    object
      [ "op" .= (2 :: Int),
        "d"
          .= object
  ...
  toJSON (Resume token session seqId) =
    object
      [ "op" .= (6 :: Int),
        "d"
          .= object
...
instance ToJSON GatewaySendable where
  toJSON (UpdateStatus (UpdateStatusOpts since game status afk)) =
    object
      [ "op" .= (3 :: Int),
        "d"
          .= object
