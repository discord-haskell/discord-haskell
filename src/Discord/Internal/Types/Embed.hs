...
createEmbed CreateEmbed {..} =
  let emptyMaybe :: T.Text -> Maybe T.Text
      ...
      embedAuthor =
        EmbedAuthor
          ...
          Nothing
      embedImage =
        (embedImageToUrl "image" <$> createEmbedImage)
          <&> \image -> EmbedImage image Nothing Nothing Nothing
      embedThumbnail =
        (embedImageToUrl "thumbnail" <$> createEmbedThumbnail)
