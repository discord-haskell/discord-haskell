  ...
  where
    ...
    firstmessage state =
      case state of
        ...
        LoopReconnect -> do
          seqId <- readIORef (gatewayHandleLastSequenceId gatewayHandle)
      ...
      case msg of
        Right (Hello interval) -> do
          ...
          -- run connection eventloop
            ...
            (runEventLoop gatewayHandle sending log)
      ...
      case eitherPayload :: Either ConnectionException GatewayReceivable of
        ...
        Left (CloseRequest code str) -> case code of
          ...
          _ -> do
            writeChan
              log
              ( "gateway - unknown websocket close code " <> T.pack (show code)
                  <> " ["
                  <> TE.decodeUtf8 (BL.toStrict str)
...
getPayloadTimeout sendablesData log = do
  ...
  res <-
    race
      (threadDelay (fromInteger ((interval * 1000 * 3) `div` 2)))
...
getPayload conn log = try $ do
  ...
  case eitherDecode msg' of
    ...
    Left err -> do
      writeChan
        log
        ( "gateway - received exception [" <> T.pack err <> "]"
            <> " while decoding "
